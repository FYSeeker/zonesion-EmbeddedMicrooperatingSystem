...
void lcd_Update(uint8_t refresh)
{
    ...
    {
        sprintf(pbuf," K1: 关闭服务 ");
        LCDShowFont16(10+(75-strlen(pbuf)*4),y+2,pbuf,strlen(pbuf)*8,PenColor,BackColor);
    }
    else
    {
        sprintf(pbuf," K1: 打开服务 ");
        LCDShowFont16(10+(75-strlen(pbuf)*4),y+2,pbuf,strlen(pbuf)*8,PenColor,BackColor);
    }
    ...
    if(TcpStatusGet() == STA_CONNECT)
        ...
        sprintf(pbuf," 状态: 已断开 ");
    LCDShowFont16(10+((75*3)-strlen(pbuf)*4),y+2,pbuf,strlen(pbuf)*8,PenColor,BackColor);
